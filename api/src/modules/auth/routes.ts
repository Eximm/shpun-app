...
function safeKeys(obj: any): string[] {
  ...
  try { return Object.keys(obj).sort(); } catch { return []; }
}
...
function dbg(req: any, label: string, extra?: Record<string, any>) {
  ...
  console.log(JSON.stringify({
    ...
    auth: { label, host, xfFor: xfFor || undefined, ip: String(req?.ip ?? ""), hasSidCookie: hasSid, ua, ...(extra ?? {}) },
  }));
}
...
function parseTelegramInitDataUser(initData: string): { tgId?: string; tgLogin?: string } {
  ...
  function tryParseUserJson(userRaw: string): any | null {
    ...
    for (let i = 0; i < 3; i++) {
      try { const obj = JSON.parse(s); return obj && typeof obj === "object" ? obj : null; }
      catch {
  ...
  try {
    ...
    return {
      ...
      tgLogin: u?.username != null ? String(u.username) : u?.first_name != null ? String(u.first_name) : undefined,
    };
  } catch { return {}; }
}
...
async function callShmTemplate(
  sessionId: string, action: string, extraParams?: Record<string, any>
): Promise<void> {
...
async function tryAttachPartner(shmSessionId: string, partnerIdRaw: any): Promise<void> {
  ...
  try { await callShmTemplate(shmSessionId, "referrals.claim", { partner_id: Math.trunc(partnerId) }); }
  catch { /* ignore */ }
}
...
async function getPasswordSetFlag(shmSessionId: string): Promise<0 | 1> {
  try {
    ...
    const isSet = stepPassword === 1 || stepPassword === "1" ||
                  legacyPwSet  === 1 || legacyPwSet  === "1";
    return isSet ? 1 : 0;
  } catch { return 1; }
}
 ...
 */
async function markPasswordStep(sessionId: string, label: string, req: any): Promise<boolean> {
  if (!sessionId) return false;
  try {
    await callShmTemplate(sessionId, "password.mark_set");
    await callShmTemplate(sessionId, "onboarding.mark", { step: "password" });
    dbg(req, `password_set:mark_done:${label}`, { sessionId: maskValue(sessionId) });
    return true;
  } catch (e: any) {
    dbg(req, `password_set:mark_failed:${label}`, { error: String(e?.message ?? e), sessionId: maskValue(sessionId) });
    return false;
  }
}
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post("/auth/telegram", async (req, reply) => {
    ...
    if (!initData) return reply.code(400).send({ ok: false, error: "init_data_required" });

    const rr = await shmTelegramWebAppAuth(initData);
    if (!rr.ok) return reply.code(mapShmAuthErrorStatus(rr.status || 502)).send({ ok: false, error: "shm_telegram_auth_failed" });

    const shmSessionId = String(rr.json?.session_id ?? "").trim();
    if (!shmSessionId) return reply.code(502).send({ ok: false, error: "no_shm_session" });

    let shmUserId = 0; let login = "";
    try { const ident = await shmGetUserIdentity(shmSessionId); shmUserId = ident.userId; login = ident.login; }
    catch { return reply.code(502).send({ ok: false, error: "shm_user_lookup_failed" }); }

    try { await tryAttachPartner(shmSessionId, body?.partner_id); } catch {}

    const localSid = reuseOrCreateSid(req);
    putSession(localSid, { shmSessionId, shmUserId, login, createdAt: Date.now(), telegramInitData: initData });

    try {
      const { tgId, tgLogin } = parseTelegramInitDataUser(initData);
      if (tgId) await callShmTemplate(shmSessionId, "auth.telegram", { telegram_id: tgId, telegram_login: tgLogin ?? "" });
    } catch {}

    const ps = await getPasswordSetFlag(shmSessionId);
    return reply.setCookie("sid", localSid, cookieOptions(req))
      .send({ ok: true, user_id: shmUserId, login, next: ps === 1 ? "home" : "set_password" });
  });

  // ── POST /auth/telegram_widget ────────────────────────────────────────────
  app.post("/auth/telegram_widget", async (req, reply) => {
    const body = (req.body ?? {}) as any;
    if (isProbablyEmptyTelegramWidgetPayload(body)) return reply.code(400).send({ ok: false, error: "missing_telegram_payload" });

    const rr = await shmTelegramWebAuth(body);
    if (!rr.ok) return reply.code(mapShmAuthErrorStatus(rr.status || 502)).send({ ok: false, error: "shm_telegram_widget_auth_failed" });
    ...
    const localSid = reuseOrCreateSid(req);
    putSession(localSid, { shmSessionId, shmUserId, login, createdAt: Date.now(), telegramWidgetPayload: pickTelegramWidgetPayload(body) });

    try { await callShmTemplate(shmSessionId, "auth.telegram", {
  ...
  app.get("/auth/telegram_widget_redirect", async (req, reply) => {
    ...
    if (isProbablyEmptyTelegramWidgetPayload(payload)) return reply.redirect("/login?e=missing_telegram_payload");
    ...
    let shmUserId = 0; let login = "";
    try { const ident = await shmGetUserIdentity(shmSessionId); shmUserId = ident.userId; login = ident.login; }
    catch { return reply.redirect("/login?e=user_lookup_failed"); }

    try { await tryAttachPartner(shmSessionId, (payload as any)?.partner_id); } catch {}

    const localSid = reuseOrCreateSid(req);
    putSession(localSid, { shmSessionId, shmUserId, login, createdAt: Date.now(), telegramWidgetPayload: pickTelegramWidgetPayload(payload) });
  ...
  app.post("/auth/password", async (req, reply) => {
    ...
    const result = await handleAuth("password", { ...body, mode, client_ip: getClientIp(req) });
    ...
    if (!shmUserId || !login) {
      try { const ident = await shmGetUserIdentity(shmSessionId); shmUserId = shmUserId || ident.userId; login = login || ident.login; }
      catch {}
    }

    if (!shmUserId) return reply.code(502).send({ ok: false, error: "shm_user_lookup_failed" });

    const localSid = reuseOrCreateSid(req);
    putSession(localSid, { shmSessionId, shmUserId, login, createdAt: Date.now() });

    dbg(req, "password_auth:done", { userId: shmUserId, mode, sid: maskValue(localSid) });
    return reply.setCookie("sid", localSid, cookieOptions(req))
      .send({ ok: true, user_id: shmUserId, login, next: "home" });
  });

  // ── POST /auth/password/set ───────────────────────────────────────────────
  // Вызывается из FirstLoginOnboardingModal при установке пароля (онбординг через бота).
  //
  // ИСПРАВЛЕННЫЙ ПОРЯДОК:
  //   1. Помечаем step_password со СТАРОЙ сессией (пока она ещё жива)
  //   2. Меняем пароль через SHM
  //   3. Пробуем re-auth чтобы обновить session_id в хранилище (best-effort)
  //   4. Если re-auth успешен — помечаем ещё раз с новой сессией (для надёжности)
  //   5. Всегда возвращаем ok:true если пароль сменился — mark некритичен
  app.post("/auth/password/set", async (req, reply) => {
    ...
    // ── Шаг 1: помечаем step_password СРАЗУ со старой сессией ────────────────
    // Делаем это ДО смены пароля пока сессия 100% жива.
    // SHM сессия валидна до момента смены пароля.
    const markedWithOld = await markPasswordStep(oldSessionId, "old_session", req);
    dbg(req, "password_set:pre_mark", { markedWithOld, oldSessionId: maskValue(oldSessionId) });

    // ── Шаг 2: меняем пароль ─────────────────────────────────────────────────
    const r = await setPassword(req, password);
    if (!r.ok) return reply.code(r.status || 400).send(r);

    // ── Шаг 3: re-auth для обновления session_id в хранилище (best-effort) ───
    // После смены пароля SHM МОЖЕТ ротировать session_id.
    // Пытаемся получить новый session_id чтобы последующие запросы работали.
    let newSessionId = "";

    // 3a. Re-auth по новому паролю (если у пользователя есть читаемый login)
    if (currentLogin && !currentLogin.startsWith("@")) {
      // Email-логин — можно re-auth по паролю
      try {
        const loginResult = await handleAuth("password", {
          mode: "login", login: currentLogin, password, client_ip: getClientIp(req),
        });
        if (loginResult.ok) {
          ...
          if (newSid) {
            ...
            if (!newUid) {
              try { const ident = await shmGetUserIdentity(newSid); newUid = ident.userId; } catch {}
            }
            if (sid) putSession(sid, { ...session, shmSessionId: newSid, shmUserId: newUid || session?.shmUserId, login: currentLogin, createdAt: session?.createdAt || Date.now() });
            newSessionId = newSid;
            dbg(req, "password_set:reauth_password_ok", { newSessionId: maskValue(newSid) });
          }
        }
      } catch (e: any) {
        dbg(req, "password_set:reauth_password_failed", { error: String(e?.message ?? e) });
      }
    }

    // 3b. Telegram re-auth (если вход был через Telegram)
    if (!newSessionId) {
      ...
      try {
        if (hasTgWebApp) {
          const rr = await shmTelegramWebAppAuth(String(session.telegramInitData).trim());
          if (rr.ok) {
            const newSid = String(rr.json?.session_id ?? "").trim();
            if (newSid) {
              try {
                const ident = await shmGetUserIdentity(newSid);
                if (sid) putSession(sid, { ...session, shmSessionId: newSid, shmUserId: ident.userId, login: ident.login || currentLogin, createdAt: session?.createdAt || Date.now() });
              } catch {}
              newSessionId = newSid;
              dbg(req, "password_set:reauth_tg_webapp_ok", { newSessionId: maskValue(newSid) });
            }
          }
        } else if (hasTgWidget) {
          ...
          if (rr.ok) {
            ...
            if (newSid) {
              ...
              newSessionId = newSid;
              dbg(req, "password_set:reauth_tg_widget_ok", { newSessionId: maskValue(newSid) });
            }
          }
        }
      } catch (e: any) {
        dbg(req, "password_set:reauth_tg_failed", { error: String(e?.message ?? e) });
      }
    }

    // ── Шаг 4: если re-auth удался — помечаем ещё раз с новой сессией ────────
    // Это гарантирует что флаг записан в актуальную сессию.
    if (newSessionId && newSessionId !== oldSessionId) {
      await markPasswordStep(newSessionId, "new_session", req);
    }

    // ── Всегда возвращаем ok:true ─────────────────────────────────────────────
    // Пароль сменился — это главное. Флаг mark некритичен для безопасности,
    // при следующем входе через Telegram сессия обновится и флаг прочитается корректно.
    // Фронт сам решает что показать пользователю после этого ответа.
    return reply.send({
      ok: true,
      password_set: 1,
      // Передаём фронту как ему перелогинить пользователя
  ...
  app.post("/auth/onboarding/mark", async (req, reply) => {
    ...
    if (!s) return reply.code(401).send({ ok: false, error: "not_authenticated" });

    const body = readJsonBody(req);
    const step = String(body?.step ?? "").trim().toLowerCase();
    if (!step) return reply.code(400).send({ ok: false, error: "step_required" });

    const allowedSteps = ["welcome", "services", "payments", "email", "password"];
    if (!allowedSteps.includes(step)) {
      return reply.code(400).send({ ok: false, error: "unknown_step", allowed: allowedSteps });
    }
    ...
    try {
      if (s.telegramInitData) {
        const rr = await shmTelegramWebAppAuth(String(s.telegramInitData).trim());
        if (!rr.ok) return reply.code(mapShmAuthErrorStatus(rr.status || 502)).send({ ok: false, error: "onboarding_reauth_failed" });
        shmSessionId = String(rr.json?.session_id ?? "").trim();
      } else if (s.telegramWidgetPayload) {
        const rr = await shmTelegramWebAuth(s.telegramWidgetPayload);
        if (!rr.ok) return reply.code(mapShmAuthErrorStatus(rr.status || 502)).send({ ok: false, error: "onboarding_reauth_failed" });
        shmSessionId = String(rr.json?.session_id ?? "").trim();
      }

      if (!shmSessionId) return reply.code(502).send({ ok: false, error: "no_shm_session_after_reauth" });

      try { const ident = await shmGetUserIdentity(shmSessionId); shmUserId = ident.userId || shmUserId; login = ident.login || login; } catch {}
      if (sidCookie) putSession(sidCookie, { ...s, shmSessionId, shmUserId, login, createdAt: s.createdAt || Date.now() });

      await callShmTemplate(shmSessionId, "onboarding.mark", { step });
      return reply.send({ ok: true, step });
    } catch (e: any) {
      return reply.code(502).send({ ok: false, error: "onboarding_mark_failed", detail: String(e?.message ?? e) });
    }
  });
  ...
  app.get("/auth/status", async (req, reply) => {
    const s = getSessionFromRequest(req) as any;
    return reply.send({ ok: true, authenticated: !!s?.shmSessionId, user_id: s?.shmUserId ?? null });
  });
