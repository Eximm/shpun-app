...
export function listReviews(options: { limit?: number; currentUserId?: number; includeAll?: boolean } = {}) {
  ...
  const reviewsSql = includeAll
    ? `
    SELECT id, user_id, user_login, display_name, rating, text, status, created_at, updated_at
    FROM reviews
    ORDER BY
  ...
  const commentsSql = includeAll
    ? `
    SELECT id, review_id, user_id, user_login, display_name, text, status, created_at, updated_at
...
export function createReview(input: {
  ...
  const info = linkDb.prepare(`
    INSERT INTO reviews (user_id, user_login, display_name, rating, text, status)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(input.userId, input.userLogin, input.displayName, rating, input.text, status);
...
export function createComment(input: {
  ...
  const status = normalizeStatus(input.status, "pending");
  const info = linkDb.prepare(`
    INSERT INTO review_comments (review_id, user_id, user_login, display_name, text, status)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(input.reviewId, input.userId, input.userLogin, input.displayName, input.text, status);
  return linkDb.prepare(`
    SELECT id, review_id, user_id, user_login, display_name, text, status, created_at, updated_at
    FROM review_comments
    WHERE id = ?
