...
//
// Internal API for the main Telegram bot (future telegram_bot.tpl).
...
// resolved on the backend and never trusted from the request body.
...
/** Returns null when authorized, otherwise sends the error response. */
...
export async function supportInternalRoutes(app: FastifyInstance) {
  ...
  app.post("/internal/support/tickets", async (req, reply) => {
    try {
      const body = readBody(req);
      if (!ensureInternalAuthorized(req, reply, body)) return;
  ...
  app.post("/internal/support/tickets/:id/messages", async (req, reply) => {
    try {
      const body = readBody(req);
      if (!ensureInternalAuthorized(req, reply, body)) return;
