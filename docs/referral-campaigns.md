...
## Рекламные ссылки без партнёрской программы

В том же разделе админки можно создать ссылку типа `campaign`: она содержит
алиас и текст комментария, но не содержит `partner_id`, процентов или бонуса.
Например, `https://app.shpun.net/?telegram_news`.

При новой регистрации API повторно разрешает алиас по серверной SQLite-таблице
и вызывает защищённое действие биллинга `campaign.claim`. Действие записывает
заданный текст только в пустое поле `user.comment` и делает контрольное чтение.
Вход уже существующего пользователя комментарий не меняет. Карточка получает
фактическое число точных совпадений комментария из биллинга. Локальный счётчик
служит резервом, увеличивается только после подтверждения записи и не более
одного раза для одной пары «ссылка + пользователь SHM».

Для работы требуется актуальная версия локального шаблона
`local-billing/shpun_app_v13_partner_active_stats.tpl` и совпадающий
`referral_secret` / `SHM_REFERRAL_SECRET`.
...
2. Принять от API доверенные параметры `first_pay` и `first_pay_campaign`.
...
5. Название кампании записать в штатный комментарий клиента; если название
   пустое — использовать алиас ссылки. Уже заполненный комментарий не
   перезаписывать.
...
- Индивидуальную ставку записывать партнёру в штатное поле карточки SHM
  (`settings.partner.income_percent`). Глобальный `billing.partner.income_percent`
  остаётся значением по умолчанию для пользователей без персональной ставки.
