  ...
  return input
    ...
    .map((raw) => {
      ...
      return {
        raw,
        protocol,
        locationKey: makeLocationKey(hashPart),
        locationLabel: normalizeLocationLabel(hashPart) || 'Unknown',
      }
    })
}
...
export default function ConnectRouter({ usi, onDone }: Props) {
  ...
  const locations = useMemo(() => {
    ...
    return filtered.filter((item) => {
      if (seen.has(item.locationKey)) return false
      seen.add(item.locationKey)
      return true
    })
  }, [routerLinks, selectedProtocol])

  const selectedLocation = useMemo(() => {
    return locations.find((item) => item.raw === selectedLinkRaw) || null
  }, [locations, selectedLinkRaw])

  const savedLocation = useMemo(() => {
    return routerLinks.find((item) => item.raw === savedLinkRaw) || null
  }, [routerLinks, savedLinkRaw])

  const canSaveConfig = useMemo(() => {
    return !configSaving && !!selectedLinkRaw && selectedLinkRaw !== savedLinkRaw && locations.length > 0
  }, [configSaving, selectedLinkRaw, savedLinkRaw, locations.length])
  ...
  async function load(opts?: { silent?: boolean }) {
    ...
    try {
      ...
      if (!silent) {
        toast.success(t('router.status_updated'), {
  ...
  async function loadConfig(opts?: { preserveSelection?: boolean }) {
    ...
    try {
      const r = (await apiFetch(`/services/${encodeURIComponent(String(usi))}/router/config`, {
        method: 'GET',
      })) as any
      ...
      const currentRaw =
        preserveSelection && selectedLinkRaw
          ? selectedLinkRaw
          : String(r?.selected_link || r?.current_link || r?.active_link || '').trim()

      const foundCurrent = parsed.find((item) => item.raw === currentRaw)

      if (foundCurrent) {
        setSelectedProtocol(foundCurrent.protocol)
        setSelectedLinkRaw(foundCurrent.raw)
        setSavedLinkRaw(foundCurrent.raw)
        return
      }

      const firstItem = parsed[0]
      setSelectedProtocol(firstItem.protocol)
      setSelectedLinkRaw(firstItem.raw)
      setSavedLinkRaw(firstItem.raw)
    } catch (e: any) {
  ...
  async function bind() {
    ...
    try {
      ...
      toast.success(t('router.bind_ok'), {
        description: `${t('router.bound')}: ${toPretty9(clean)}`,
      })
    } catch (e: any) {
  ...
  async function saveConfig() {
    ...
    try {
      ...
      setSavedLinkRaw(selectedLinkRaw)
      setLocationOpen(false)

      toast.success('Сервер переключён', {
        description: `${locationLabel} · ${protocolText}`,
      })
      ...
      const msg = errMessage(e, t('router.config.save_error'))
      setConfigError(msg)
      toast.error(t('router.config.save_error'), { description: msg })
    } finally {
  ...
  useEffect(() => {
    ...
    const exists = locations.some((item) => item.raw === selectedLinkRaw)
    if (!exists) {
      setSelectedLinkRaw(locations[0].raw)
    }
  }, [locations, selectedLinkRaw])
        ...
        <div className="card__body">
          <div className="row so__spaceBetween">
            <div>
              ...
              <div className="p">
                {hasBound
                  ? 'Выберите протокол и локацию, затем сохраните изменения.'
                  : t('router.hint')}
              </div>
            </div>
          ...
          {loading ? (
            <div className="section">
              <div className="p">{t('router.loading')}</div>
            </div>
          ) : null}

          {error ? (
            <div className="pre cr__mt10">{error}</div>
          ) : null}
            ...
            <>
              ...
              {hasBound ? (
                <>
                  <div className="section">
                    <div className="field">
                      <label className="field__label">Протокол</label>
                      <div className="actions actions--2">
                        <button
                          className={`btn ${selectedProtocol === 'ss' ? 'btn--primary' : ''}`}
                          onClick={() => setSelectedProtocol('ss')}
                          type="button"
                        ...
                        <button
                          className={`btn ${selectedProtocol === 'vless' ? 'btn--primary' : ''}`}
                          onClick={() => setSelectedProtocol('vless')}
                          type="button"
                          disabled={configSaving || configLoading}
                        >
                  ...
                  {configError ? (
                    <div className="pre cr__mt10">{configError}</div>
                  ) : null}
                  ...
                  {!configLoading && routerLinks.length > 0 ? (
                    <>
                      <div className="section" ref={locationRef}>
                        <div className="field">
                          ...
                          >
                            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {selectedLocation?.locationLabel || 'Выберите сервер'}
                            </span>
                            <span style={{ marginLeft: 12, opacity: 0.7, flex: '0 0 auto' }}>
                              {locationOpen ? '▴' : '▾'}
                            </span>
                          </button>
                        ...
                        {locationOpen ? (
                          <div
                            className="card so__cardFlat cr__mt10"
                            style={{
                              boxShadow: 'none',
                      ...
                      <div className="section">
                        <div className="actions actions--2">
                          <button
                            ...
                            type="button"
                          >
                            {configSaving ? t('router.config.saving') : 'Сохранить сервер'}
                          </button>

                          <button
                            className="btn btn--danger so__btnFull"
                            onClick={unbind}
