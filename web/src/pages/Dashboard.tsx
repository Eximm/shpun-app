...
function chipByServiceStatus(s: ServiceStatus) {
  switch (s) {
    case "active":
      return { label: "Active", cls: "chip chip--ok" };
    case "pending":
      return { label: "Processing", cls: "chip chip--warn" };
    case "not_paid":
      return { label: "Not paid", cls: "chip chip--bad" };
    case "blocked":
      return { label: "Blocked", cls: "chip chip--bad" };
    case "error":
      return { label: "Error", cls: "chip chip--bad" };
    case "removed":
      return { label: "Removed", cls: "chip" };
    case "init":
      return { label: "Init", cls: "chip" };
    default:
      return { label: s || "—", cls: "chip" };
  }
}
...
export function Dashboard() {
  ...
  const accountState = useMemo(() => {
    ...
    if (summary.notPaid > 0)
      return { cls: "chip chip--bad", text: "Payment required" };
    if (summary.active > 0) return { cls: "chip chip--ok", text: "Active" };
    if (summary.total > 0)
      return { cls: "chip chip--warn", text: "No active services" };
  ...
  const nearestExpire = useMemo(() => {
    const candidates = items
      ...
      .sort((a, b) => {
        const ta = a.expireAt
          ? new Date(a.expireAt).getTime()
          : Number.POSITIVE_INFINITY;
        const tb = b.expireAt
          ? new Date(b.expireAt).getTime()
          : Number.POSITIVE_INFINITY;
        return ta - tb;
      });
      ...
      <div className="card" style={{ marginTop: 16 }}>
        <div className="card__body">
                        ...
                        {s.daysLeft !== null ? (
                          <>
                            ...
                            {s.daysLeft >= 0
                              ...
                              : `${Math.abs(s.daysLeft)} days overdue`}
                          </>
                        ) : null}
          ...
          <div className="row" style={{ marginTop: 12 }}>
            <button className="btn" onClick={() => nav("/services")}>
              Open services
            </button>
          </div>
        </div>
