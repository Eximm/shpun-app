// web/src/pages/help/ServicesRouter.tsx
    ...
    () => [
      {
        ...
        body: t(
          ...
          "Shpun Router подключает ваш роутер к сети Shpun SDN System. Весь домашний трафик идёт через защищённый VPN-туннель — без настройки на каждом устройстве отдельно."
        ),
        note: t(
          "servicesRouter.what.note",
          "Никаких SSH, терминала и ручных конфигов — всё делается через интерфейс LuCI и Telegram-бота."
        ),
        bullets: [
      ...
      },
      {
        ...
        note: t(
          ...
          "Обычно вся настройка занимает несколько минут после установки пакета."
        ),
        steps: [
        ...
        <div className="card__body">
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
            <div style={{ minWidth: 0 }}>
              <h1 className="h1">🛰️ {t("servicesRouter.page.title", "Shpun Router")}</h1>
              <p className="p">
                {t(
              ...
              <div className="row" style={{ marginTop: 12, gap: 8, flexWrap: "wrap" }}>
                ...
                <span className="chip">OTA-обновления</span>
              </div>
            </div>
            <button
              className="btn"
        ...
        <div className="card" key={i} style={{ marginTop: 12 }}>
          <div className="card__body">
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 2 }}>
              ...
              <div className="h1" style={{ fontSize: 18 }}>{b.title}</div>
            </div>

            {b.body ? <p className="p">{b.body}</p> : null}

            {b.note ? (
              <div className="pre" style={{ marginTop: 10 }}>{b.note}</div>
            ) : null}
              ...
              <div className="kv" style={{ marginTop: 12 }}>
                {b.steps.map((s, idx) => (
                  <div className="kv__item" key={idx} style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                    ...
                    <span style={{ paddingTop: 2, lineHeight: 1.45 }}>{s}</span>
                  </div>
                ))}
              </div>
            ) : null}
              ...
              <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                {b.bullets.map((s, idx) => (
                  <div key={idx} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                    <span style={{ flexShrink: 0, marginTop: 3, color: "var(--accent-2)", fontWeight: 900 }}>›</span>
